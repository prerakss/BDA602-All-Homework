...
def main():
    df, predictors, response = get_test_data_set("mpg")
    ...
    # CATEGORICAL - CONTINUOUS PAIR CORRELATION MATRIX AND TABLE

    def categorical_continuous_pairs():

        cat_cont_df = pd.DataFrame(
            columns=["Categorical predictor", "Continuous predictor", "Correlation"]
        )
        for i in cat_predictors:
            for j in cont_predictors:
                correlation = cat_cont_correlation_ratio(df[i], df[j])
                cat_cont_df.loc[len(cat_cont_df)] = [i, j, correlation]
                ...
                fig_2.update_layout(
                    title=i + "variable Violin plot",
                    xaxis_title=f"{i}",
                    yaxis_title=f"{j}",
                )

        pivot = cat_cont_df.pivot_table(
            "Correlation", ["Categorical predictor"], "Continuous predictor"
        )

        # Correlation matrix for cont-cat
        fig = ff.create_annotated_heatmap(
            z=pivot.to_numpy(), x=list(pivot.columns), y=pivot.index.to_list()
        )
        filename = "Categorical_Continuous_correlation_matrix.html"
        fig.write_html(file=filename, include_plotlyjs="cdn")

        cat_cont_df.to_csv("Categorical_categorical_pairs_correlation_table.csv")

    # CATEGORICAL - CATEGORICAL PAIR CORRELATION MATRIX AND TABLE

    def categorical_categorical_pairs():
        ...
        for i in cat_predictors:
            for j in cat_predictors:
                if i != j:
                    correlation = cat_correlation(df[i], df[j])
                    cat_cat_df.loc[len(cat_cat_df)] = [i, j, correlation]

        # Correlation matrix
        pivot = cat_cat_df.pivot_table(
            "Correlation", ["Categorical predictor 1"], "Categorical predictor 2"
        )
        fig = ff.create_annotated_heatmap(
            z=pivot.to_numpy(), x=pivot.index.to_list(), y=pivot.index.to_list()
        )
        filename = "Categorical_categorical_correlation_matrix.html"
        fig.write_html(file=filename, include_plotlyjs="cdn")

        cat_cat_df.to_csv("Categorical_categorical_pairs_correlation_table.csv")

    # BRUTE FORCE TABLES

    def two_variable_mean_response_brute_force():

        # 3 different tables for 3 pairs
        ...
        cont_cat_brute_force_table = pd.DataFrame(
            columns=[
                "predictor_1",
                "predictor_2",
                "unweighted mean of response",
        ...
        # Cat-cat brute force
        for p1 in cat_predictors:
            for p2 in cat_predictors:
                if p1 != p2:
                    df_temp = df
                    p1_binning, p2_binning = "Bins:" + p1, "Bins:" + p2

                    df_temp[p1_binning] = df_temp[p1]
                    df_temp[p2_binning] = df_temp[p2]
                    ...
                    mean_values_merged["squared diff"] = (
                        1
                        / (len(set(df_temp[p1])) * len(set(df_temp[p2])))
                        * np.power(mean_values_merged["diff with mean of response"], 2)
                    )
        ...
        for p1 in cont_predictors:
            for p2 in cat_predictors:
                ...
                cont_cat_brute_force_table.loc[len(cont_cat_brute_force_table)] = [
                    p1,
                    p2,
                    unweigh_diff,
                    weigh_diff,
            ...
            print(cont_cat_brute_force_table.to_csv("cont_cat_brute_force_table.csv"))
    ...
    def normal_mean_of_response():
        ...
        df_temp = df
        for p in cat_predictors:
            w = df_temp.groupby(df_temp[p]).count()[response] / sum(
                df_temp.groupby(df_temp[p]).count()[response]
            )
            w = w.reset_index()

            _mean = np.mean(df[response])
            bin = df.groupby(df_temp[p]).apply(np.mean)
            ...
            cat_diff_table.to_csv("normal_mean_of_response_categorical.csv")
    ...
    def homework_4_plots():
        ...
        if response_type == "Continuous":
            # Cont response, cont pred
            for p in cont_predictors:
                fig = px.scatter(x=df[p], y=df[response], trendline="ols")
                fig.update_layout(
                    title=f"Scatter plot for {p}", xaxis_title=p, yaxis_title=response
                )
                filename = f"{response}_{p}_scatterplot_hw4.html"
            ...
            # Cont response, cat pred
            for p in cat_predictors:

                pred, group_labels = df[p].factorize()
                pred = pd.DataFrame(pred)

                hist_data = pd.concat([pred, df[response]], axis=1)

                fig = px.histogram(hist_data, group_labels)
                fig.update_layout(
                    title=f"Relationship Between {response} and {p}",
                    xaxis_title=response,
                    yaxis_title=p,
                )
                filename = f"{response}_{p}_histogramplot_hw4.html"
                plots_df.loc[len(plots_df)] = [
                    response,
                    p,
                    os.getcwd() + "/" + filename,
            ...
            # Cat response, cat pred
            for p in cat_predictors:

                conf_matrix = confusion_matrix(
                    df[p].astype(str), df[response].astype(str)
                )
                fig_4 = go.Figure(
                ...
                filename = f"{response}_{p}_heatplot_hw4.html"

                fig_4.write_html(file=filename, include_plotlyjs="cdn")
                plots_df.loc[len(plots_df)] = [
                    response,
                    p,
                    os.getcwd() + "/" + filename,
